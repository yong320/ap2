...
root_agent = RetryingLlmAgent(
    ...
    3. shipping_address_collector から配送先住所が返ってきたら、
       その住所をユーザーに丁寧に表示してください。表示後、必ず次の確認メッセージを追加してください：

      "この住所で間違いありませんか？よろしければ『はい』とお答えください。"

         ここで重要なルールがあります：
         ・ユーザーが「はい」「OK」「同意します」「大丈夫です」など、
         明確に肯定の返答をするまでは次のステップへ進んではいけません。
         ・肯定の返答があるまでは必ず同じ質問を続けてください。
         ・ユーザーが肯定した場合にのみ、次の処理を実行してください。
